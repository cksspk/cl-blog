  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="标签名" prop="title">
        <el-input v-model="queryParams.title" placeholder="请输入标签名" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['blog:tag:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['blog:tag:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['blog:tag:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="标签名" prop="title">
          <el-input v-model="form.title" placeholder="请输入标签名" />
        </el-form-item>
...
<script>
import { createTag, updateTag, deleteTag, getTag, getTagPage, exportTagExcel } from "@/api/blog/tag";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除博客标签编号为"' + id + '"的数据项?').then(function() {
          return deleteTag(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有博客标签数据项?').then(() => {
          this.exportLoading = true;
          return exportTagExcel(params);
        }).then(response => {
          this.$download.excel(response, '博客标签.xls');
